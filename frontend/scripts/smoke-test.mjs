const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5200';
...
const adminCookie = await login('admin@ai-automation-agency.local', 'admin123');
const managerCookie = await login('manager@ai-automation-agency.local', 'manager123');
const analystCookie = await login('analyst@ai-automation-agency.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/chatbot-builder', analystCookie);
...
const records = await expectJson('/api/entities/chatbot-builder', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/chatbot-builder/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/chatbot-builder/approve`, {
  method: 'POST',
  headers: {
...
console.log('AI Automation Agency Suite smoke passed');
